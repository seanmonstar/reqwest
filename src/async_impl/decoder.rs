...
use futures::{task::{Poll, Context}, Future, Stream};
use hyper::{HeaderMap};
...
impl Future for Pending {
    ...
    fn poll(self: &mut Self, cx: &mut Context) -> Poll<Self::Output> {
        ...
        match body_state {
            StreamState::Eof => Ok(Poll::Ready(Inner::PlainText(Body::empty()))),
            StreamState::HasMore => Ok(Poll::Ready(Inner::Gzip(Gzip::new(body))))
        }
    }
}
