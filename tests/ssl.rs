...
fn test_custom_ca_hostname_verification_disabled() {
    let mut listening = start_server("tests/certificates/server-wrong.hostname.com.pfx");
    let client = get_client(HostnameVerification::Disabled);
    let mut resp = match client.get("https://localhost:12345").unwrap().send() {
        Ok(resp) => resp,
        Err(e) => {
...
fn test_custom_ca_hostname_verification_enabled() {
    let mut listening = start_server("tests/certificates/server-localhost.pfx");
